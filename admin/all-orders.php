...
if(isset($_POST['ok'])){
    ...
    $select = "UPDATE order_form SET un_read = '$mark_as_read'  WHERE customer_id = '$user_id' && status = 'Approved'";
    $query_run = mysqli_query($conn, $select);

    if($query_run){
            header('Location:approved-order.php');
    }
}

$search_order_details = display_orders();
...
<!-- Header -->
    <header class="header">    
            <nav class="nav container flex">
                    ...
                    <?php 
                                $user_id = $_SESSION['ID'];
                                $inbox = $conn->query("SELECT * FROM `order_form` where status = 'To Claim' ")->num_rows;
                                ?>
                    <div class="menu-content">
                            <ul class="menu-list flex">
                                    ...
                                    <li>
                                        <span id="state1" countto="70"><?= number_format($inbox) ?></span> 
                                        <a href="to-claim.php" class="nav-link active-navlink">To Claim</a>
                                    </li>
                    ...
                    <table class="content-table">
                        ...
                        <?php
                                     ...
                                     //   $query_run = mysqli_query($conn, $query);
                                        ...
                                        while ($row = mysqli_fetch_array( $search_order_details))
                                        {
                                            ?>
                                                <td><a href="view-pending-order.php?id=<?= $row['ID']; ?>" class="btn btn-success button-update">view</a></td>      
